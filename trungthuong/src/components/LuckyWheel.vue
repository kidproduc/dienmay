<template>
    <div class="luckywheel">
            ...
            <g ref="active" class="active">
                <g>
                    ...
                    <path
                        ...
                        transform="translate(0)" fill="#ccc" />
                </g>
            </g>
        </svg>
        ...
        <button ref="btnPlay" class="btn">Play</button>
        <button ref="btnSlowMo" class="btn">Slow Motion</button>
    </div>
</template>
...
export default {
    setup() {
        ...
        const btnSlowMo = ref(null);

        onMounted(() => {
            const wheelElement = wheel.value;
            const activeElement = active.value;
            const btnPlayElement = btnPlay.value;
            const btnSlowMoElement = btnSlowMo.value;

            let currentRotation;
            let lastRotation = 0;
            let tolerance;
            const deg = getRandomInt(360, 1080);
            console.log(deg);

            const indicator = gsap.timeline();
            const spinWheel = gsap.timeline();
            indicator.to(activeElement, { rotation: -10, transformOrigin: "65% 36%", ease: "power1.out", duration: 0.13 })
                             .to(activeElement, { rotation: 3, ease: "power4.out", duration: 0.13 })
                             .add("end");

            spinWheel.to(wheelElement, { rotation: deg, transformOrigin: "50% 50%", ease: "power4.out", duration: 5, onUpdate: () => {
                currentRotation = Math.round(gsap.getProperty(wheelElement, "rotation"));
            ...
            btnSlowMoElement.addEventListener('click', () => {
